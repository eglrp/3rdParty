...
struct SbaData1
{
	...
	double x0, y0;
	double f; //标定的固定焦距
	unsigned char* flags; //标示哪些相机是控制点的，用来修改sba的参数增量大小用
};
...
{
	//修改sba
	struct SbaData1* global_parm = (struct SbaData1*) adata;
	    ...
	    _dblzero(dpa, mcon*cnp); /* no change for the first mcon camera params */
            ...
            if(issolved){
                ...
                for(i=0, dp_L2=0.0; i<nvars; ++i){
                    pdp[i]=p[i] + (tmp=dp[i]);
					////修改sba
					//if (0 == i % cnp && i <= (m - 1) * cnp && global_parm->flags[i / cnp] < 0)
					//{
					//	pdp[i]=p[i] + 0.001 * (tmp=dp[i]);
					//} 
					//else
					//{
					//	pdp[i]=p[i] + (tmp=dp[i]);
